...
This vignette shows the summary figures for the SOM results. The code used to create these summary figures may be found in `code/functions.R`. Figures showing the synoptic states for variables for all of the nodes at once are shown first, followed by multi-panel summary plots for each individual node, which are listed below by their number. Use the table of contents on the left of the screen to move quickly between variables and specific nodes of interest as desired.
